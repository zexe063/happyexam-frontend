...
export default function Avatar(){
    ...
    return (
       ...
       <div className="w-[412px] md:w-[800px] h-full  rounded-xl md:border-[2px] md:border-solid md:border-gray-200 flex justify-between flex-col md:flex-row gap-3">

        <div className=" relative w-full h-[30%] md:w-[50%] md:h-full border-r-[2px] border-solid border-gray-200 flex justify-center items-end rounded-xl"  style={{backgroundColor:`${avatarCollection[selectedAvatarIndex-1]?.bgcolor}`}}>
            <img src={`/avatar/${avatarCollection[selectedAvatarIndex-1]?.id}.svg`} className=" w-[170px] h-[220px]  md:w-[500px] md:h-[500px]"></img>
          ...
          <div className=" w-full h-[70%] md:w-[50%] md:h-full overflow-auto grid grid-cols-3 place-items-center ">
            ...
            avatarCollection.map((item,index)=>{
               return( 
                 ...
                 <div key={item.id} className={`w-[112px] h-[112px] flex justify-center items-end rounded-xl ${selectedAvatarIndex===index+1 && "outline outline-2 outline-offset-2 outline-blue-700"} cursor-pointer `} style={{backgroundColor:`${item.bgcolor}` }} onClick={()=>setSelectedAvatarIndex(index+1)}>
            <img src={`/avatar/${item.id}.svg`} className=" w-[100px] h-[100px]"></img>
        </div>
               )
            })
            }
