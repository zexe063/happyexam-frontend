...
export default function Avatar(){
    ...
    const AvatarId  = Number(user.Avatar.imageId.match(/[0-9]/gm)[0])
     const [selectedAvatarIndex , setSelectedAvatarIndex] = useState(AvatarId)
    console.log(user.Avatar.imageId)
    
     function AvatarSave(){
         naviagte("/profile")
        dispatch(UserAvatarSave(AvatarImage[selectedAvatarIndex-1]))
     }
    ...
    return (
       ...
       <div className="w-[412px] md:w-[800px] h-full  rounded-xl border-[2px] border-solid border-border_grey flex justify-between flex-col md:flex-row gap-3">
        ...
        <div className=" relative w-full h-[30%] md:w-[50%] md:h-full border-r-[2px] border-solid border-border_grey flex justify-center items-center rounded-xl"  style={{backgroundColor:`${AvatarImage[selectedAvatarIndex-1].bgcolor}`}}>
            <img src={`../Avatar/${AvatarImage[selectedAvatarIndex-1].imageId}`} className=" w-full h-full md:w-[500px] md:h-[500px]"></img>
          ...
          <div className=" w-full h-[70%] md:w-[50%] md:h-full overflow-auto grid grid-cols-3 place-items-center ">
            ...
            <img src={`../Avatar/${item.imageId}`} className=" w-full h-full "></img>
        </div>
               )
            })
            }
